...
class Backup:
    ...
    def getShardInfo(self):
        ...
        if shard_doc:
            for shard in shard_doc:
                self.shard_info.append(str(shard).split('\'')[3])
            return True   
    ...
    def prepareForBackup(self):
        ...
        for shard in self.shard_info:
            second_node = []
            second_node = self.getSecondary(shard)
            if second_node.__len__() == 0:
                 return False
            self.second_node_used[str(shard).split('/')[0]] = second_node[0]
    ...
    def fullBackupShard(self):
         ...
         if check_dir(dir):
             for shard in self.shard_info:
                ...
                status, date = commands.getstatusoutput("date +%Y-%m-%d")
                backup_dir = dir + '/' + date+ '/' + str(shard).split('/')[0]
                commands.getstatusoutput("mkdir -p "+backup_dir)
                last_oplog_dir = dir + '/oplog_json/'+ str(shard).split('/')[0]
                commands.getstatusoutput("mkdir -p "+last_oplog_dir)
               
                #get the second node of this shard
                second = self.second_node_used[str(shard).split('/')[0]]
                if second == null:
                    return False
                ...
                cmd_last_oplog_record = self.config.mongo_bin_dir + "/mongo "+  \
                          str(second) + "/local "     \
                          " --quiet " +              \
                          " --eval 'db.oplog.rs.find().sort({\"$natural\":-1}).limit(1)' "+\
                ...
                print "finished full back " + str(second) + "."
    ...
    def incBackupShard(self):
         ...
         if check_dir(dir) and check_dir(full_dir):
            for shard in self.shard_info:
                ...
                last_oplog_dir = full_dir + '/oplog_json/'+ str(shard).split('/')[0]
                ...
                cmd_dump = "mongodump --host " + second + " --out " + backup_dir + " --db local --collection oplog.rs --query " + query
                status, output = commands.getstatusoutput(cmd_dump)
                ...
                print "finished inc back " + str(second) + "."

    def getSecondary(self, shard):
        secondaryList = []
        print shard + ':getting secondaries ... '
        shard_client = pymongo.MongoClient(str(shard).split('/')[1])
